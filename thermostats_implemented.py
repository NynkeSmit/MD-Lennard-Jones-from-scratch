...
def andersen_thermostat(velocities, masses, temperature, nu, dt, kB=1.0, rng=None):
    ...
    # For colliding particles, sample new velocities from MB distribution
    # HINT: Use the initialize_velocities_maxwell_boltzmann function or
    # implement the sampling directly here
...
def langevin_thermostat(velocities, masses, temperature, nu, dt, kB=1.0, rng=None):
    ...
    # Update velocities using the Langevin equation
    # v_new = c * v_old + noise_amplitude * sigma * R
    velocities_new = c * velocities + noise_amplitude * sigma * random_noise
...
def berendsen_thermostat(velocities, masses, temperature, nu, dt, kB=1.0, remove_com=True):
    ...
    # STUDENT TASK: Handle edge case where T_current is very small
    # HINT: If T_current is close to zero, we might get division by zero
    ...
    if remove_com and v_com is not None:
        ...
        velocities_new = scaling_factor * (velocities - v_com) + v_com
    else:
        velocities_new = scaling_factor * velocities
